...
// Removed import of 'react' to resolve module error
...
export default function HomePageMaster() {
  ...
  const loadHomepageData = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (
          transformedData.about.image &&
          transformedData.about.image.startsWith('blob:')
        ) {
          console.warn(
            'Found blob URL in database, clearing it:',
            transformedData.about.image
          );
          transformedData.about.image = '';
        }
        if (
          ...
          transformedData.hero.backgroundImage.startsWith('blob:')
        ) {
          console.warn(
            'Found blob URL in database, clearing it:',
            transformedData.hero.backgroundImage
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      if (uploadResult.success && uploadResult.data?.url) {
        ...
        toast.warning(
          `${file.name} selected for preview. Note: Upload API not available, using temporary preview.`.replace(
            "'",
            '&apos;'
          )
        );
      ...
      updateContent(section, field, fileUrl);
      toast.warning(
        `${file.name} selected for preview. Upload failed, using temporary preview.`.replace(
          "'",
          '&apos;'
        )
      );
  ...
  const addFaq = async () => {
    if (!newFaq.question || !newFaq.answer) {
      toast.warning(
        'Please fill in both question and answer fields'.replace("'", '&apos;')
      );
      return;
    }
    ...
    try {
      ...
      const result = await apiService.post('/homepage/faqs', faqRequest);
      ...
      toast.error(
        'Failed to add FAQ. Please try again.'.replace("'", '&apos;')
      );
  ...
  const saveEditingFaq = async () => {
    ...
    try {
      ...
      const originalFaq = contentData.faqs.find(
        (f: any) => f.id === editingFaqId
      );
      if (originalFaq?.question !== editingFaqData.question) {
        const result = await apiService.put(`/homepage/faqs/${cleanId}`, {
          question: editingFaqData.question
        });
        if (!result.success) {
          toast.error(
            result.error ||
              'Failed to update FAQ question'.replace("'", '&apos;')
          );
          return;
        }
      }
      ...
      if (originalFaq?.answer !== editingFaqData.answer) {
        ...
        if (!result.success) {
          toast.error(
            result.error || 'Failed to update FAQ answer'.replace("'", '&apos;')
          );
          return;
        }
      }
      ...
      toast.error(
        'Failed to update FAQ. Please try again.'.replace("'", '&apos;')
      );
    }
  };
  ...
  const deleteFaq = async (faqId: string) => {
    try {
      ...
      const result = await apiService.delete(`/homepage/faqs/${cleanId}`);

      if (result.success) {
        // Remove from local state
        setContentData((prev: any) => ({
          ...prev,
          faqs: prev.faqs.filter((faq: any) => faq.id !== faqId)
        }));
        setHasChanges(true);
          ...
          <div className='flex space-x-2'>
            ...
            >
              {isSaving ? (
                ...
                <>
                  {/* Removed Save icon due to missing import */}
                ...
                <Card key={index}>
                  ...
                  <CardContent className='space-y-4'>
                    <div className='space-y-3'>
                      <div className='h-3 animate-pulse rounded bg-gray-100'></div>
              ...
              <Card>
                <CardHeader>
                  <div>
                    ...
                    <CardDescription>
                      Main banner and call-to-action area
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <div className='rounded-lg border-2 border-dashed border-gray-300 p-6 text-center'>
                      ...
                      {contentData.hero.backgroundImage ? (
                        ...
                        <>
                          {/* Removed Upload icon due to missing import */}
              ...
              <Card>
                <CardHeader>
                  <div>
                    ...
                    <CardDescription>
                      Brief introduction about the platform
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <Textarea
                      ...
                      onChange={(e: any) =>
                        updateContent('about', 'content', e.target.value)
                      }
                      className='min-h-[120px]'
                    />
                  </div>
                        ...
                        <div className='space-y-2'>
                              ...
                              if (parent) {
                                ...
                                if (!errorMsg) {
                                  ...
                                  placeholder.className =
                                    'error-placeholder flex items-center justify-center h-24 w-24 mx-auto bg-gray-100 border-2 border-dashed border-gray-300 rounded';
                                  placeholder.innerHTML =
                                    '<span class="text-xs text-red-500">Image not found</span>';
                                  parent.insertBefore(
                                    placeholder,
                                    e.currentTarget
                                  );
                          ...
                          {contentData.about.image.startsWith('blob:') && (
                            <p className='text-xs text-orange-600'>
                              ⚠️ This is a temporary preview. Click &quot;Save
                              Changes&quot; to upload permanently.
                            </p>
                          )}
                        </div>
                      ) : (
              ...
              <Card>
                ...
                <CardContent className='space-y-4'>
                  <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                    ...
                    <div className='space-y-2'>
                      <Label htmlFor='contact-support-email'>
                        Support Email
                      </Label>
                      <Input
                    ...
                    <div className='space-y-2'>
                      ...
                      <Input
                        ...
                        onChange={(e: any) =>
                          updateContent('contact', 'salesEmail', e.target.value)
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className='space-y-2'>
                    ...
                    <Textarea
                      ...
                      onChange={(e: any) =>
                        updateContent('contact', 'description', e.target.value)
                      }
