...
const STEPS = [
  {
    ...
    title: 'Basic Info',
    icon: BookOpen,
    description: 'Topic details and information'
  },
  {
    id: 2,
    title: 'Modules & Videos',
    icon: Video,
    description: 'Edit modules with video content'
  },
  {
    id: 3,
    title: 'Audience & Tags',
    icon: Users,
    description: 'Target audience and tags'
  },
  {
    id: 4,
    title: 'Settings & Publish',
    icon: Settings,
    description: 'Pricing and publishing options'
  }
];
...
export default function EditTopicPage() {
              ...
              return {
                ...
                duration: String(
                  video.duration || video.durationSeconds
                    ? Math.ceil(video.durationSeconds / 60)
                    : ''
                ),
                videoFile: null,
                videoUrl: video.videoUrl || '',
    ...
    const fetchTopic = async () => {
      try {
        ...
        const result = await apiService.get<any>(
          API_ENDPOINTS.TOPICS.BY_ID(topicId)
        );
        ...
        if (result.success && result.data) {
          ...
          if (modulesWithVideos.length > 0) {
            ...
            console.log(
              '📂 Auto-expanding modules with videos:',
              modulesWithVideos
            );
          }
  ...
  const addModule = () => {
    ...
    console.log(
      '✅ Module added successfully. Total modules:',
  ...
  const updateModule = (
    moduleIndex: number,
    field: keyof LocalModule,
    value: any
  ) => {
    console.log(
      `🔄 Updating module ${moduleIndex}, field: ${field}, value:`,
      value
    );
    ...
    setFormData((prev: LocalTopicFormData | null) => {
      ...
      const updatedModules = prev.modules.map(
        (module: LocalModule, index: number) =>
  ...
  const removeModule = (moduleIndex: number) => {
    ...
    setFormData((prev: LocalTopicFormData | null) => {
      ...
      const filteredModules = prev.modules.filter(
        (_: LocalModule, index: number) => index !== moduleIndex
      );
  ...
  const handleBulkVideoUpload = async (
    ...
    toast.success(
      `Added ${videoFiles.length} video(s) successfully! Save the topic to upload them.`
    );
  };
  ...
  const handleQuickVideoUpload = (videos: any[]) => {
    ...
    const currentVideoCount =
      formData.modules[targetModuleIndex]?.videos.length || 0;
    ...
    const newVideos: LocalVideo[] = videos.map((video, index) => ({
      ...
      videoUrl:
        video.url || (video.file ? URL.createObjectURL(video.file) : ''),
    ...
    toast.success(
      `Added ${videos.length} video(s) to ${formData.modules[targetModuleIndex]?.title || 'Module 1'}`
    );
  };
  ...
  const handleModuleVideoUpload = (moduleIndex: number, videos: any[]) => {
    ...
    try {
      const currentVideoCount =
        formData.modules[moduleIndex]?.videos.length || 0;
      ...
      toast.success(
        `Added ${videos.length} video(s) to ${formData.modules[moduleIndex]?.title || `Module ${moduleIndex + 1}`}`
      );
    } catch (error) {
    ...
    try {
      ...
      if (module && !String(module.id).startsWith('new-') && topicId) {
        ...
        formDataForUpload.append(
          'order',
          String(formData.modules[moduleIndex].videos[videoIndex].order)
        );
        ...
        try {
          ...
          toast.error(
            'Upload failed. Video will be uploaded when you save the topic.'
          );
        }
      }
    } catch (error) {
  ...
  const handleUpdate = async (status?: 'draft' | 'published') => {
    ...
    console.log(
      '📋 Current form data before update:',
    ...
    try {
      ...
      const updatedModules = await Promise.all(
        formData.modules.map(async (module, moduleIndex) => {
          ...
          const videosToUpload = module.videos.filter(
            (video) => video.videoFile && !video.uploadedUrl
          );
          const urlBasedVideos = module.videos.filter(
            (video) =>
              !video.videoFile &&
              video.videoUrl &&
          ...
          const alreadyUploadedVideos = module.videos.filter(
            (video) =>
          ...
          const oversizedVideos = videosToUpload.filter(
            (video) => video.videoFile && video.videoFile.size > MAX_FILE_SIZE
          );

          if (oversizedVideos.length > 0) {
            console.error(
              `❌ Oversized videos in module ${module.title}:`,
              oversizedVideos.map((v) => ({
                title: v.title,
                size: v.videoFile
            ...
            toast.error(
              `Some videos in ${module.title} exceed the 500MB limit. Please compress them first.`
            );
            return {
              ...module,
              videos: module.videos.map((video) => ({
          ...
          try {
            ...
            if (videosToUpload.length > 0) {
              ...
              toast.info(
                `Starting upload of ${videosToUpload.length} video(s) for ${module.title}...`
              );
              ...
              while (retryCount <= maxRetries) {
                try {
                  ...
                  retryCount++;
                  console.log(
                    `📤 Upload attempt ${retryCount} failed for module: ${module.title}`,
                    uploadError
                  );
              ...
              if (!uploadResult) {
                toast.error(
                  'Failed to upload videos after multiple attempts. Please try again.'
                );
                setIsLoading(false);
                return;
              }
            ...
            const cleanedAlreadyUploaded = alreadyUploadedVideos.map(
              (video) => ({
                ...video,
                videoFile: null
              })
              ...
              for (const urlVideo of urlBasedVideos) {
                try {
                  const videoPayload = {
                    ...
                    videoType: urlVideo.videoUrl.includes('youtube')
                      ? 'youtube'
                      : 'external',
                  ...
                  if (createResult.success && createResult.data) {
                    ...
                    finalVideos.push({
                      ...
                      uploadedUrl:
                        createResult.data.videoUrl || urlVideo.videoUrl,
                      thumbnail:
                        createResult.data.thumbnailUrl ||
      ...
      if (result.success) {
        ...
        try {
          const refreshResult = await apiService.get<any>(
            API_ENDPOINTS.TOPICS.BY_ID(topicId)
          );
        ...
        toast.error(
          result.error || 'Failed to update topic. Please try again.'
        );
      }
    } catch (error) {
                ...
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  <div className='space-y-2'>
                    ...
                    <div className='flex gap-2'>
                      ...
                      >
                        <SelectTrigger
                          className={`h-9 ${!formData.category ? 'border-red-300' : ''}`}
                        >
                  ...
                  <div className='space-y-2'>
                    ...
                    <div className='flex gap-2'>
                      ...
                      <Button
                        ...
                        size='sm'
                        onClick={() =>
                          formData.category && refreshSubcategories()
                        }
                ...
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  <div className='space-y-2'>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setFormData((prev) =>
                          prev ? { ...prev, difficulty: value as any } : prev
                        )
                      }
                    >
                      <SelectTrigger
                        className={`h-9 ${!formData.difficulty ? 'border-red-300' : ''}`}
                      >
                    ...
                    <AccordionContent>
                      <div className='space-y-2'>
                        <RichTextEditor
                          ...
                          onChange={(value) =>
                            setFormData((prev) =>
                              prev ? { ...prev, prerequisites: value } : prev
                            )
                          }
            ...
            <Card>
              <CardHeader className='pb-4'>
                <CardTitle className='flex items-center justify-between'>
                  ...
                  <Button
                    ...
                    variant='outline'
                    size='sm'
                    className='hover:border-primary hover:bg-primary/5 border-dashed'
                  >
              ...
              <CardContent className='space-y-4'>
                {formData.modules.length === 0 ? (
                  <div className='rounded-lg border border-dashed border-gray-300 bg-gradient-to-b from-gray-50 to-white py-12 text-center'>
                    ...
                    <p className='mx-auto mb-6 max-w-md text-gray-500'>
                      Organize your content into modules. Each module can
                      contain multiple videos, documents, and other learning
                      materials.
                    </p>
                  ...
                  <div className='space-y-4'>
                    {formData.modules.map((module, moduleIndex) => {
                      ...
                      const completedVideos = module.videos.filter(
                        (v) =>
                          v.title &&
                          (v.videoFile || v.videoUrl || v.uploadedUrl)
                      ).length;
                                ...
                                <div className='flex h-full items-center justify-between'>
                                  <div className='flex items-center gap-2'>
                                    ...
                                    <div className='text-left'>
                                      <h4 className='text-xs leading-tight font-medium text-gray-900'>
                                        {module.title ||
                                          `Module ${module.order}`}
                                      </h4>
                                      <div className='flex items-center gap-2'>
                                        ...
                                        {isCompleteModule && (
                                          <Badge
                                            variant='secondary'
                                            className='h-4 bg-green-100 px-1 py-0 text-xs text-green-700'
                                          >
                                            Complete
                                          </Badge>
                                        )}
                                  ...
                                  <div className='flex items-center gap-1'>
                                    <Button
                                      ...
                                      className='h-5 w-5 p-0 text-red-500 hover:bg-red-50 hover:text-red-700'
                                    >
                                      ...
                                      <VideoUploadModal
                                        trigger={
                                          <Button
                                            variant='outline'
                                            size='sm'
                                            className='bg-primary/5 hover:bg-primary/10 border-primary/20 h-7 px-3 text-xs'
                                          >
                                        ...
                                        <div className='grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8'>
                                          {module.videos.map(
                                            (video, videoIndex) => (
                                              ...
                                              >
                                                <div className='relative flex aspect-video items-center justify-center overflow-hidden rounded border bg-gray-100'>
                                                  {video.videoUrl ? (
                                                    <video
                                                      src={video.videoUrl}
                                                  ...
                                                  {/* Video order */}
                                                  <div className='absolute top-1 right-1 rounded bg-black/70 px-1 text-xs text-white'>
                                                    {video.order ||
                                                      videoIndex + 1}
                                                  </div>
                                                </div>
                                                ...
                                                {/* Video title - smaller */}
                                                <p className='mt-1 truncate text-center text-xs text-gray-600'>
                                                  {video.title ||
                                                    `Video ${videoIndex + 1}`}
                                                </p>
                                              </div>
                    ...
                    <div className='pt-4'>
                      <Button
                        onClick={addModule}
                        variant='outline'
                        className='hover:border-primary hover:bg-primary/5 w-full border-2 border-dashed py-4 transition-all'
                      >
                ...
                <CardContent className='space-y-4'>
                  <div className='flex items-center justify-between rounded-lg border p-4'>
                    <div className='space-y-1'>
                      ...
                      <p className='text-sm text-gray-500'>
                        Make this topic free for all users
                      </p>
                    </div>
                    <Switch
                      id='isFree'
                  ...
                  <div className='flex items-center justify-between rounded-lg border p-4'>
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setFormData((prev) =>
                          prev ? { ...prev, featured: checked } : prev
                        )
                      }
                    />
                  </div>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <RichTextEditor
                      ...
                      onChange={(value) =>
                        setFormData((prev) =>
                          prev ? { ...prev, metaDescription: value } : prev
                        )
                      }
              ...
              onClick={() => {
                ...
                if (formData?.modules) {
                  formData.modules.forEach((module, index) => {
                    console.log(
                      `🐛 DEBUG: Module ${index} (${module.title}):`,
                      {
                        id: module.id,
                        videosCount: module.videos.length,
                        videos: module.videos.map((v) => ({
                ...
                toast.info(
                  `Debug: ${formData?.modules?.length || 0} modules, Title: ${formData?.title || 'N/A'}`
                );
              }}
            >
              Debug State
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (String(moduleWithVideos.id).startsWith('new-')) {
                  toast.error(
                    'Cannot test upload on new module - save the topic first'
                  );
                  return;
                }

                console.log(
                  '🧪 Testing video upload for module:',
                  moduleWithVideos.title
                );
                console.log('🧪 Module ID:', moduleWithVideos.id);
                console.log('🧪 Topic ID:', topicId);

                const videosToTest = moduleWithVideos.videos.filter(
                ...
                try {
                  ...
                  if (result.success) {
                    toast.success(
                      'Test upload successful! Check console for details.'
                    );
                  } else {
