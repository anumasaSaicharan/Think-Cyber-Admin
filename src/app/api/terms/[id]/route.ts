  ...
  try {
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Terms and conditions not found'
        },
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Missing required fields: title, content, version, language, status'
        },
        { status: 400 }
      );
    }
    ...
    if (!versionRegex.test(body.version)) {
      return NextResponse.json(
        {
          success: false,
          error: 'Version must be in format X.Y (e.g., 1.0, 2.1)'
        },
        { status: 400 }
      );
    }
    ...
    const response = await apiService.put<any>(
      API_ENDPOINTS.TERMS.BY_ID(termsId),
      {
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to update terms and conditions'
        },
        {
    ...
    // Transform snake_case to camelCase
    const updatedTerms = response.data
      ? transformTermsData(response.data)
      : null;
  ...
  try {
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to delete terms and conditions'
        },
        { status: response.error?.includes('not found') ? 404 : 500 }
      );
    }
    ...
    const deletedTerms = response.data
      ? transformTermsData(response.data)
      : null;
