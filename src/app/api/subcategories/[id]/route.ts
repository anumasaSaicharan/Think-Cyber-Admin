...
export async function GET(
  ...
  try {
    ...
    const response = await apiService.get(
      API_ENDPOINTS.SUBCATEGORIES.BY_ID(subCategoryId)
    );
...
export async function PUT(
  ...
  try {
    ...
    const response = await apiService.put<any>(
      API_ENDPOINTS.SUBCATEGORIES.UPDATE(subCategoryId),
      {
        name: body.name.trim(),
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to update sub-category'
        },
        {
...
export async function DELETE(
  ...
  try {
    ...
    const response = await apiService.delete<any>(
      API_ENDPOINTS.SUBCATEGORIES.DELETE(subCategoryId)
    );
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to delete sub-category'
        },
        { status: response.error?.includes('not found') ? 404 : 500 }
      );
    }
