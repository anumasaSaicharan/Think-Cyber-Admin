  ...
  try {
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to fetch subcategories for category'
        },
        { status: 500 }
      );
    }
    ...
    const stats = {
      total: subcategories.length,
      active: subcategories.filter((sc: SubCategory) => sc.status === 'Active')
        .length,
      draft: subcategories.filter((sc: SubCategory) => sc.status === 'Draft')
        .length,
      inactive: subcategories.filter(
        (sc: SubCategory) => sc.status === 'Inactive'
      ).length,
      totalTopics: subcategories.reduce(
        (sum: number, sc: SubCategory) => sum + (sc.topicsCount || 0),
        0
      )
    };
