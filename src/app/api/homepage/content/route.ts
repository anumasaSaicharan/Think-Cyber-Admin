...
export async function GET(request: Request) {
  try {
    ...
    const response = await apiService.get(
      `${API_ENDPOINTS.HOMEPAGE.CONTENT}?language=${language}`
    );
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to fetch homepage content'
        },
        { status: 500 }
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    const response = await apiService.post(
      API_ENDPOINTS.HOMEPAGE.CONTENT,
      body
    );
    ...
    if (!response.success) {
      return NextResponse.json(
        {
          success: false,
          error: response.error || 'Failed to create/update homepage content'
        },
        { status: 500 }
      );
    }
