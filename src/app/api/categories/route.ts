  ...
  try {
    ...
    try {
        ...
        return NextResponse.json({
          ...
          stats: {
            ...
            draft: minimalFallback.filter(
              (cat: Category) => cat.status === 'Draft'
            ).length,
            inactive: minimalFallback.filter(
              (cat: Category) => cat.status === 'Inactive'
            ).length,
            totalTopics: minimalFallback.reduce(
              (sum: number, cat: Category) => sum + (cat.topicsCount || 0),
              0
            )
          },
      ...
      const stats = response.data?.stats || {
        total: categories.length,
        active: categories.filter((cat: Category) => cat.status === 'Active')
          .length,
        draft: categories.filter((cat: Category) => cat.status === 'Draft')
          .length,
      ...
      return NextResponse.json({
        ...
        stats,
        message: fetchAll
          ? `Fetched all ${categories.length} categories from backend`
          : `Fetched ${categories.length} categories from backend (page ${page})`
      });
    } catch (error) {
      // Return minimal fallback data
      return NextResponse.json({
...
export async function POST(request: Request) {
  try {
    ...
    const category = response.data
      ? transformCategoryData(response.data)
      : null;
